          ...
          <Suspense
            fallback={
              <div role="status" className="w-full py-2">
                <svg
                  aria-hidden="true"
                  className="w-9 h-9 text-gray-300 animate-spin fill-blue-600 mx-auto"
                  viewBox="0 0 100 101"
                  fill="none"
            ...
            <Await resolve={fetchUnreadNotifications()}>
              {(resolvedData) => {
                return resolvedData && resolvedData.length > 0 ? (
                  resolvedData.map((notification, index) => {
                    if (index < 3) {
                      return (
                        <NotificationItem
                          key={index}
      ...
      () =>
        resolve(
          DUMMY_NOTIFICATIONS.filter(
            (notification) => !notification.isMarkedAsRead
          )
        ),
      1000
    );
