...
const getAllUsersProfiles = async(req, res) => {
    try {
        const [result] = await pool.query(
            "SELECT userId, email, username, first_name, last_name, date_of_birth, phone_number, gender, address, role, registration_date, last_time_login,image, status FROM users"
        );
        setTimeout(() => {
