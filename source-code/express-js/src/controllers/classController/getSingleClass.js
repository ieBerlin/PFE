  ...
  try {
    ...
    if (userRole === "member") {
      ...
      const [enrollmentResult] = await pool.query(
        "SELECT status FROM enrollmentrequests WHERE class_id = ? AND applicant_user_id = ?",
        [classId, userId]
      );

      const [instructorResults] = await pool.query(
        'SELECT userId,email, CONCAT(first_name," ", last_name) AS name FROM users WHERE userId = ?',
      ...
      return res.status(200).json({ message: "Class status", classData });
    }
  } catch (error) {
