...
async function createNotificationToAdmin(req, res) {
    try {
        ...
        if (result.length === 0) {
            return res.status(404).json({ success: false, error: 'No admins found to notify' });
        }
...
module.exports = createNotificationToAdmin;